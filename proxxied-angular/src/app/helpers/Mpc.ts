...
export function inferCardNameFromFilename(filename: string): string {
  ...
  const cleaned = beforeParen.replace(/[_-]+/g, ' ').replace(/\s+/g, ' ').trim();
  return cleaned;
}
...
export function tryParseMpcSchemaXml(raw: string): MpcItem[] | null {
  ...
  for (const fc of fronts) {
    ...
    const name = (
      looksLikeFilename ? inferCardNameFromFilename(nameText) : nameText || query || 'Custom Art'
    ).trim();
...
export function parseMpcText(raw: string): MpcItem[] {
  ...
  for (const line of lines) {
    ...
    const qty = Number.isFinite(parseInt(tokens[0], 10)) ? parseInt(tokens[0], 10) : 1;
